...
### Requirement: Build all supported platform binaries
The build system SHALL produce agent binaries for all supported OS/architecture combinations in a single command.

#### Scenario: Build all targets
- **WHEN** the user runs `make build-all`
- **THEN** the system produces binaries for linux (amd64, arm64, arm, mips, mipsle, mips64, mips64le), freebsd (amd64, arm64), and darwin (amd64, arm64) in the `bin/` directory
