    ...
    <div className="max-w-6xl">
      ...
      <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl shadow-[var(--shadow)]">
        ...
        <div className="divide-y divide-[var(--border)]">
          {recentLogs.length === 0 ? (
            <p className="px-5 py-8 text-sm text-[var(--muted-foreground)] text-center">
              No activity yet
            </p>
          ) : (
            recentLogs.map((log) => (
...
function ActionBadge({ action }: { action: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center justify-center w-7 h-7 rounded-md text-xs font-medium ${color}`}
    >
      {isAdd ? "+" : isRemove ? "-" : "~"}
    </span>
  );
